...
function calculateMetrics(rows) {
    ...
    rows.forEach(row => {
        ...
        totalIncome += parseFloat(row[4]) || 0;  // Monthly Income
        totalFinancialAid += parseFloat(row[5]) || 0;  // Financial Aid
    });

    const avgMonthlyIncome = rows.length ? totalIncome / rows.length : 0;
